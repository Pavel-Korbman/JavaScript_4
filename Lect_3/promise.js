...
Promise.all([
    ...
    new Promise((resolve, reject) => setTimeout(() => resolve(2), 3500)),
...
//💡 Promise.race()
// Метод Promice.race() берёт из массива промисов первый выполнившийся (по скорости)
// промис и возвращает его результат. Остальные промисы будут проигнорированы.
//💡Promise.any()
// Promice.any ждёт результат первого успешно выполнившегося промиса, остальные
// игнорируется. Если все промисы будут с ошибками, то вернётся ошибка объекта
// AggregateError.
//💡Promise.allSettled()
...
// Вы разрабатываете приложение, которое отправляет запросы разным серверам и хотите определить - какой сервер отвечает быстрее. Напишите функцию checkServerResponse(urls), которая принимает массив URL-адресов серверов и возвращает Promise. Promise должен быть выполнен с URL-адресом сервера, который первым ответил на запрос.
...
checkServerResponse(urls)
    .then((fastestServer) => {
        console.log('Самый быстрый сервер: ', fastestServer); // Самый быстрый сервер:  https://api.github.com/
    })
    .catch((error) => {
